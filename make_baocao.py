...
Khung trinh bay tong hop tu cac bao cao khoa truoc (chi tiet nhat):
  Moi chuong trinh o Chuong 2-3 gom:
    - Phan tich thiet ke (mo ta + BANG mo ta chuc nang)
    - Chuong trinh (mo ta cai dat + ma nguon day du)
    - Ket qua thuc thi (KHUNG placeholder anh + caption)
Co: bia, danh muc hinh, danh muc bang, muc luc, danh muc viet tat, 4 chuong.
"""
import os
import docx
...
def code_block(text):
    for ln in text.rstrip('\n').split('\n'):
        ...
        p.paragraph_format.line_spacing = 1.0; p.paragraph_format.left_indent = Inches(0.2)
...
def _seq_field(p, seqname):
    fb = OxmlElement('w:fldChar'); fb.set(qn('w:fldCharType'), 'begin')
    it = OxmlElement('w:instrText'); it.set(qn('xml:space'), 'preserve')
    it.text = ' SEQ %s \\* ARABIC ' % seqname
...
def caption(label, seqname, desc):
    p = doc.add_paragraph(style='Caption')
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.paragraph_format.space_before = Pt(2)
    r = p.add_run(label + ' '); r.italic = True; r.font.size = Pt(12); r.font.name = 'Times New Roman'
    _seq_field(p, seqname)
    r2 = p.add_run('. ' + desc); r2.italic = True; r2.font.size = Pt(12); r2.font.name = 'Times New Roman'
...
def figure(desc, hint):
    ...
    caption('Hình', 'Hình', desc)
    doc.add_paragraph().paragraph_format.space_after = Pt(2)

def desc_table(rows, cap):
    """Bang mo ta chuc nang: rows = [(chuc nang, mo ta), ...] (khong gom header)."""
    t = doc.add_table(rows=len(rows) + 1, cols=2); t.style = 'Table Grid'
    t.alignment = WD_TABLE_ALIGNMENT.CENTER
    hdr = t.rows[0].cells
    hdr[0].text = 'Chức năng'; hdr[1].text = 'Mô tả'
    for c in hdr:
    ...
    for i, (a, b) in enumerate(rows, 1):
        cs = t.rows[i].cells
        cs[0].text = a; cs[1].text = b
        for c in cs:
            c.paragraphs[0].runs[0].font.size = Pt(12)
    caption('Bảng', 'Bảng', cap)
    doc.add_paragraph().paragraph_format.space_after = Pt(2)
...
def program(title, intro, functions, table_cap, impl, code_src, result_desc, result_hint):
    """Render mot chuong trinh theo khung: PTTK + bang + chuong trinh + ket qua."""
    h3(title)
    h4('Phân tích thiết kế')
    body(intro)
    desc_table(functions, table_cap)
    h4('Chương trình')
    for para in impl:
        body(para)
    if code_src.endswith(('.sh', '.c', 'Makefile')) or '/' in code_src:
        code_file(code_src)
    else:
        code_block(code_src)
    h4('Kết quả thực thi chương trình')
    figure(result_desc, result_hint)
...
for line in ['Giảng viên hướng dẫn :  ............................',
             ...
             'Khóa                 :  ............................']:
    p = doc.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.paragraph_format.space_after = Pt(4); p.add_run(line).font.size = Pt(13)
center('', space=40); center('Hà Nội, 2026', 13, True)
doc.add_page_break()

# ============================================================
# DANH MUC HINH ANH / BANG BIEU / MUC LUC
# ============================================================
center('DANH MỤC HÌNH ẢNH', 15, True, space=10)
field(' TOC \\h \\z \\c "Hình" ', 'Nhấn Ctrl+A rồi F9 để cập nhật danh mục hình ảnh.')
doc.add_page_break()

center('DANH MỤC BẢNG BIỂU', 15, True, space=10)
field(' TOC \\h \\z \\c "Bảng" ', 'Nhấn Ctrl+A rồi F9 để cập nhật danh mục bảng biểu.')
doc.add_page_break()
...
for i, (a, b) in enumerate(abbr):
    c0, c1 = t.rows[i].cells; c0.text = a; c1.text = b
    if i == 0:
        for c in (c0, c1):
            c.paragraphs[0].runs[0].bold = True; _set_shd(c._tc, 'D9E2F3')
doc.add_page_break()
...
# ============================================================
h1('LỜI MỞ ĐẦU')
body('Trong thời đại bùng nổ công nghệ thông tin hiện nay, các phần mềm mã nguồn mở '
     'ngày càng đóng vai trò quan trọng. Linux là một trong những hệ điều hành mã '
     'nguồn mở tiêu biểu nhất: miễn phí, ổn định, bảo mật cao và có thể tùy biến sâu. '
     'Linux hiện diện ở khắp nơi, từ máy chủ, điện toán đám mây, thiết bị nhúng cho '
     'đến điện thoại di động (nhân Android) và siêu máy tính.')
...
body('Linux là một hệ điều hành mã nguồn mở (open-source), chạy trên hầu hết các '
     ...
     'trợ bởi cộng đồng mã nguồn mở (GNU). Nhờ tính mở, người dùng được quyền sử dụng '
     'miễn phí, sao chép, nghiên cứu và sửa đổi mã nguồn. Hầu hết các bản Linux đều '
     'hỗ trợ nhiều ngôn ngữ lập trình, đặc biệt công cụ GCC cho phép biên dịch và '
...
figure('Hệ điều hành Linux', 'logo/ảnh minh họa hệ điều hành Linux.')
...
sub('► Application (Ứng dụng): lớp trên cùng, nơi người dùng cài đặt và chạy các ứng '
    'dụng phục vụ nhu cầu của mình.')
...
sub('► Nhược điểm: không phải hệ điều hành thời gian thực nên không phù hợp một số '
    'ứng dụng đòi hỏi xử lý khẩn cấp; thiếu chuẩn thống nhất nên hỗ trợ kỹ thuật còn '
    'hạn chế.')
...
body('Chương này trình bày phần lập trình shell (Phần 1) và lập trình C (Phần 2) để '
     'quản lý hệ thống Linux. Mỗi chương trình được phân tích thiết kế thành các '
     'chức năng, tổ chức dạng menu cho phép người dùng chọn chức năng bằng số, kèm mã '
     'nguồn và kết quả thực thi.')

h2('2.1. Lập trình Shell quản lý hệ thống')

program(
    '2.1.1. Chương trình quản lý file',
    'Chương trình quanly_file.sh được phân tích thành các chức năng quản lý file/thư '
    'mục cơ bản, điều hướng bằng một menu vòng lặp. Các chức năng chính:',
    [('Tạo thư mục/file', 'Nhận tên từ bàn phím, dùng lệnh mkdir -p / touch để tạo mới.'),
     ('Đổi tên', 'Nhận tên cũ và tên mới, dùng lệnh mv để đổi tên.'),
     ('Sao chép', 'Nhận nguồn và đích, dùng lệnh cp -r để sao chép.'),
     ('Di chuyển', 'Nhận nguồn và đích, dùng lệnh mv để di chuyển.'),
     ('Xóa', 'Có bước xác nhận (y/n) trước khi dùng rm -rf để xóa.'),
     ('Điều hướng', 'Dùng lệnh cd để chuyển thư mục (.. = thư mục cha).'),
     ('Liệt kê', 'Dùng ls hoặc ls -lah để hiển thị nội dung thư mục.'),
     ('Phân quyền', 'Nhận quyền (vd 755), dùng lệnh chmod để cấp quyền.'),
     ('Nén', 'Dùng tar -zcvf để nén thành tệp .tar.gz.')],
    'Bảng mô tả các chức năng của chương trình quản lý file (shell)',
    ['Hàm chính dùng vòng lặp while kết hợp case để điều hướng tới từng chức năng. '
     'Mỗi chức năng nhận tham số nhập từ bàn phím bằng read và thực thi lệnh hệ thống '
     'tương ứng, sau đó in thông báo [OK]/[Lỗi]. Mã nguồn đầy đủ:'],
    '1_shell/quanly_file.sh',
    'Kết quả thực thi chương trình quản lý file',
    'terminal chạy ./quanly_file.sh, thao tác tạo/đổi tên/xóa thư mục, in [OK].')

program(
    '2.1.2. Chương trình lập lịch tác vụ',
    'Chương trình laplich_tacvu.sh quản lý các tác vụ định kỳ của người dùng thông '
    'qua crontab. Cú pháp crontab gồm 5 trường: phút, giờ, ngày, tháng, thứ. Các '
    'chức năng:',
    [('Liệt kê tác vụ', 'Dùng crontab -l để in danh sách tác vụ hiện có.'),
     ('Thêm tác vụ', 'Nối dòng lịch mới vào crontab hiện tại qua crontab -.'),
     ('Xóa một tác vụ', 'Xóa dòng theo số thứ tự bằng sed rồi nạp lại crontab.'),
     ('Xóa toàn bộ', 'Dùng crontab -r để xóa tất cả tác vụ (có xác nhận).')],
    'Bảng mô tả các chức năng của chương trình lập lịch tác vụ',
    ['Chương trình kiểm tra hợp lệ số thứ tự dòng trước khi sửa/xóa, dùng tệp tạm '
     '(mktemp) để thao tác an toàn trên crontab. Mã nguồn:'],
    '1_shell/laplich_tacvu.sh',
    'Kết quả thực thi chương trình lập lịch tác vụ',
    'terminal thêm một tác vụ crontab rồi liệt kê lại danh sách.')

program(
    '2.1.3. Chương trình thiết lập thời gian hệ thống',
    'Chương trình thoigian_hethong.sh cho phép xem và thiết lập thời gian hệ thống, '
    'cũng như đồng bộ giờ từ máy chủ NTP. Các thao tác thay đổi giờ cần quyền sudo:',
    [('Xem thời gian', 'Dùng lệnh date để hiển thị ngày, giờ, múi giờ hiện tại.'),
     ('Đặt giờ', 'Dùng sudo date -s để đặt giờ theo định dạng HH:MM:SS.'),
     ('Đặt ngày', 'Dùng sudo date -s để đặt ngày theo định dạng YYYY-MM-DD.'),
     ('Đặt múi giờ', 'Dùng sudo timedatectl set-timezone để đổi múi giờ.'),
     ('Đồng bộ NTP', 'Cài và chạy ntpdate pool.ntp.org để đồng bộ giờ qua mạng.')],
    'Bảng mô tả các chức năng của chương trình thiết lập thời gian',
    ['Mỗi chức năng sau khi thực thi đều gọi lại hàm hiển thị thời gian để người dùng '
     'kiểm chứng kết quả. Mã nguồn:'],
    '1_shell/thoigian_hethong.sh',
    'Kết quả thực thi chương trình thiết lập thời gian hệ thống',
    'terminal xem giờ hiện tại và đồng bộ NTP thành công.')

program(
    '2.1.4. Chương trình cài đặt và gỡ bỏ phần mềm',
    'Chương trình caidat_goibo.sh tự động hóa việc quản lý gói phần mềm trên Ubuntu '
    'bằng apt và dpkg. Các chức năng:',
    [('Kiểm tra gói', 'Dùng dpkg -s để kiểm tra một gói đã được cài hay chưa.'),
     ('Cài đặt', 'Dùng apt-get update và apt-get install để cài gói mới.'),
     ('Gỡ bỏ', 'Dùng apt-get remove để gỡ bỏ gói đã cài.'),
     ('Cập nhật', 'Dùng apt-get update và upgrade để nâng cấp hệ thống.'),
     ('Tìm kiếm', 'Dùng apt-cache search để tìm gói theo từ khóa.')],
    'Bảng mô tả các chức năng của chương trình cài đặt/gỡ bỏ phần mềm',
    ['Trước khi cài, chương trình kiểm tra gói đã tồn tại chưa để tránh cài lại; '
     'tương tự, chỉ gỡ khi gói thực sự đã được cài. Mã nguồn:'],
    '1_shell/caidat_goibo.sh',
    'Kết quả thực thi chương trình cài đặt/gỡ bỏ phần mềm',
    'terminal cài một gói (vd: tree) rồi gỡ bỏ gói đó.')

h3('2.1.5. Menu tổng hợp')
body('Kịch bản menu.sh đóng vai trò trình đơn chính, gọi đến bốn chương trình con ở '
     'trên, giúp người dùng truy cập nhanh mọi chức năng từ một điểm vào duy nhất.')
code_file('1_shell/menu.sh')

h2('2.2. Lập trình C quản lý tiến trình, file, socket và network')

program(
    '2.2.1. Quản lý tiến trình',
    'Chương trình quanly_tientrinh.c minh họa quản lý tiến trình trên Linux thông qua '
    'các lời gọi hệ thống. Các chức năng:',
    [('Liệt kê tiến trình', 'Dùng lệnh ps (qua system) để hiển thị các tiến trình.'),
     ('Tạo tiến trình con', 'Dùng fork() tạo tiến trình con, execl() chạy lệnh.'),
     ('Gửi tín hiệu', 'Dùng kill(pid, sig) để gửi tín hiệu bất kỳ tới tiến trình.'),
     ('Tiêu diệt tiến trình', 'Dùng kill(pid, SIGKILL) để buộc dừng tiến trình.'),
     ('Chờ tiến trình con', 'Dùng waitpid() để cha chờ con kết thúc và lấy mã thoát.')],
    'Bảng mô tả các chức năng của chương trình quản lý tiến trình',
    ['Khi tạo tiến trình con, tiến trình cha dùng waitpid() để chờ và in mã thoát của '
     'con. Mã nguồn:'],
    '2_c/quanly_tientrinh.c',
    'Kết quả thực thi chương trình quản lý tiến trình',
    'terminal liệt kê tiến trình và tạo một tiến trình con chạy lệnh (vd: ls -l).')

program(
    '2.2.2. Quản lý file',
    'Chương trình quanly_file.c thao tác với file bằng các lời gọi hệ thống và thư '
    'viện chuẩn của C. Các chức năng:',
    [('Kiểm tra tồn tại', 'Dùng stat() để kiểm tra một file có tồn tại hay không.'),
     ('Tạo file', 'Dùng fopen(..., "w") để tạo file rỗng.'),
     ('Đọc file', 'Dùng fopen(..., "r") và fgetc() để in nội dung file.'),
     ('Ghi thêm', 'Dùng fopen(..., "a") để ghi thêm nội dung vào cuối file.'),
     ('Xóa file', 'Dùng remove() để xóa file.'),
     ('Xem quyền', 'Đọc st_mode từ stat() để hiển thị quyền dạng rwx.'),
     ('Cấp quyền', 'Dùng chmod() với quyền nhập dạng bát phân (vd 755).'),
     ('Liệt kê thư mục', 'Dùng opendir()/readdir() để liệt kê file trong thư mục.')],
    'Bảng mô tả các chức năng của chương trình quản lý file (C)',
    ['Chương trình tổ chức dạng menu lặp, mỗi lựa chọn gọi hàm tương ứng. Mã nguồn:'],
    '2_c/quanly_file.c',
    'Kết quả thực thi chương trình quản lý file (C)',
    'terminal tạo file, ghi nội dung, đọc lại và xem quyền của file.')

program(
    '2.2.3. Thông tin mạng (Network)',
    'Chương trình network.c lấy thông tin các giao diện mạng của máy. Chức năng:',
    [('Liệt kê giao diện', 'Dùng getifaddrs() để lấy danh sách các giao diện mạng.'),
     ('Hiển thị địa chỉ IP', 'Dùng getnameinfo() để in địa chỉ IPv4/IPv6 từng giao diện.')],
    'Bảng mô tả các chức năng của chương trình network',
    ['Chương trình duyệt danh sách giao diện, lọc các họ địa chỉ AF_INET (IPv4) và '
     'AF_INET6 (IPv6) rồi in tên giao diện kèm địa chỉ. Mã nguồn:'],
    '2_c/network.c',
    'Kết quả thực thi chương trình network',
    'terminal in danh sách giao diện mạng (lo, eth0/ens33…) kèm địa chỉ IP.')

h3('2.2.4. Lập trình Socket (mô hình Client – Server TCP)')
h4('Phân tích thiết kế')
body('Cặp chương trình server.c và client.c minh họa giao tiếp hai chiều qua socket '
     'TCP theo mô hình Client – Server. Các bước và chức năng chính:')
desc_table(
    [('Server: tạo socket', 'socket(AF_INET, SOCK_STREAM, 0) tạo socket TCP.'),
     ('Server: bind & listen', 'bind() gắn socket vào cổng 8080, listen() lắng nghe.'),
     ('Server: accept', 'accept() chấp nhận kết nối đến từ client.'),
     ('Client: connect', 'connect() kết nối tới địa chỉ IP và cổng của server.'),
     ('Trao đổi dữ liệu', 'Hai bên dùng send()/read() để gửi và nhận tin nhắn.'),
     ('Kết thúc', 'Gõ “bye” để đóng kết nối và thoát chương trình.')],
    'Bảng mô tả các bước của chương trình Socket Client – Server')
h4('Chương trình')
body('Mã nguồn phía Server:')
code_file('2_c/server.c')
body('Mã nguồn phía Client (sửa SERVER_IP nếu chạy khác máy):')
...
h4('Kết quả thực thi chương trình')
figure('Kết quả thực thi chương trình Socket',
       'hai cửa sổ terminal: ./server và ./client nhắn tin qua lại.')
doc.add_page_break()
...
body('Mọi module đều có một hàm khởi tạo (đánh dấu __init, đăng ký qua module_init) '
     ...
     'vào qua macro module_param. Các macro MODULE_LICENSE, MODULE_AUTHOR, '
     'MODULE_DESCRIPTION khai báo giấy phép, tác giả và mô tả.')
desc_table(
    [('module_init / __init', 'Đăng ký và đánh dấu hàm chạy khi nạp module.'),
     ('module_exit / __exit', 'Đăng ký và đánh dấu hàm chạy khi gỡ module.'),
     ('module_param', 'Khai báo tham số truyền vào khi nạp module (insmod).'),
     ('printk', 'Ghi thông điệp ra nhật ký nhân, xem bằng dmesg.'),
     ('MODULE_LICENSE…', 'Khai báo giấy phép, tác giả, mô tả của module.')],
    'Bảng mô tả các thành phần cơ bản của một module nhân')
...
figure('Kết quả thực thi chương trình module nhân',
       'terminal: insmod module rồi chạy dmesg hiển thị log; và dòng "Tam biet" khi rmmod.')
doc.add_page_break()
...
body('Cấu trúc của Buildroot gồm: Makefile (chứa sẵn script build OS với các cấu '
     'hình khác nhau), Config.in (chứa các tùy chọn cấu hình), .config (lưu các tùy '
     'chọn đã cài đặt). Sau khi build, ta có đầy đủ các thành phần cơ bản của một hệ '
     'điều hành: Kernel, Root filesystem, Bootloader và các thành phần phụ trợ khác.')
figure('Cấu trúc Buildroot', 'sơ đồ các thành phần đầu ra của Buildroot.')
...
body('Hệ thống Linux nhúng ARM 64-bit cần có: hệ thống tệp gốc, hình ảnh nhân và ứng '
     'dụng người dùng; ngoài ra cần một trình biên dịch chéo và một trình mô phỏng '
     '(QEMU) khi thử nghiệm trên nền tảng không phải ARM. Buildroot tự động hóa toàn '
     'bộ quá trình này. Các bước thực hiện:')
...
figure('Tải xuống và tạo cấu hình Buildroot',
       'terminal chạy git clone buildroot và make qemu_aarch64_virt_defconfig.')
body('Bước 2 — Trong menuconfig, chọn thêm các gói/tùy chọn phục vụ GPIO:')
sub('+ Kernel  >  Linux Kernel Tools  >  gpio')
sub('+ Target packages  >  Libraries  >  Hardware handling  >  libgpiod (bật '
    '“install tools”)')
sub('+ Target packages  >  Networking applications  >  openssh')
figure('Cấu hình các gói trong menuconfig',
       'màn hình menuconfig chọn libgpiod, gpio tools, openssh.')
body('Bước 3 — Cấu hình nhân để bật driver GPIO của phần cứng giả lập rồi lưu lại:')
code_block('''make linux-menuconfig
# Chon: Device Drivers > GPIO Support >
#       Memory mapped GPIO drivers > PrimeCell PL061 GPIO support''')
figure('Cấu hình driver Memory mapped GPIO (PL061)',
       'màn hình linux-menuconfig bật PrimeCell PL061 GPIO support.')
...
figure('Hệ thống Linux ARM khởi động trên QEMU',
       'màn hình QEMU boot xong và đăng nhập vào shell root.')
...
figure('Biên dịch chương trình hello.c bằng cross-compiler',
       'terminal biên dịch hello.c bằng aarch64-linux-gcc.')
figure('Kết quả thực thi hello.c trên QEMU',
       'trong QEMU chạy ./hello in ra "Hello QEMU".')
...
body('Để giao tiếp và quản lý các đường GPIO trên hệ thống Linux mô phỏng, ta sử '
     'dụng thư viện libgpiod cùng các công cụ dòng lệnh đã tích hợp qua Buildroot. '
     'Đăng nhập với tài khoản root rồi thực thi lệnh gpiodetect để liệt kê các chip '
     'GPIO:')
...
body('Toàn bộ 8 chân (line 0–7) đang ở trạng thái chưa đặt tên (unnamed), chưa sử '
     ...
     '(active-high).')
figure('Kết quả chạy gpiodetect và gpioinfo',
       'terminal trong QEMU chạy gpiodetect và gpioinfo.')
...
main(int argc, char *argv[])
{
    ...
    output_line = gpiod_chip_get_line(output_chip, 3);     /* chan 3 */
...
figure('Quá trình chuyển tệp thực thi sang QEMU qua HTTP',
       'terminal trong QEMU dùng wget tải blink và pl061-change.')
...
figure('Kết quả thay đổi GPIODATA ghi nhận từ pl061-change',
       'terminal hiển thị các dòng GPIODATA=8/0 luân phiên.')
...
figure('Kết quả thực thi chương trình đồng bộ đa luồng gpio',
       'terminal trong QEMU chạy ./gpio in ra 10 dòng Event.')
doc.add_page_break()
...
for i, ref in enumerate([
    ...
    p = doc.add_paragraph('[%d] %s' % (i, ref)); p.paragraph_format.space_after = Pt(4)
